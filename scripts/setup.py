...
if len(sys.argv) < 1:
    print("Usage: python setup.py path/to/dump/dir")
    sys.exit(1)
...
with open(
    ...
    path.join(dump_dir, "jung_archetypes_store.dat"), mode="wb"
) as f_jung_archetypes_store, open(
    path.join(dump_dir, "jung_interpretations_store.dat"), mode="wb"
) as f_jung_interpretations_store, open(
    path.join(dump_dir, "personality_types_store.dat"), mode="wb"
) as f_personality_types_store, open(
    path.join(dump_dir, "freud_interpretations_store.dat"), mode="wb"
